...
# Sampling procedure
S <- 500000 # number of samples
B <- 230000  # burn-in samples
...
T_oss = (S/5)/2
x <- posterior_sample_1$values[1:T_oss]
y <- posterior_sample_1$values[(T_oss+1):S]
ks.test(x,y)
...
# Kolmogorov-Smirnov test
x <- posterior_sample_2$values[1:T_oss]
y <- posterior_sample_2$values[(T_oss+1):S]
ks.test(x,y)
